...
idt_descriptor_t IDT_DESC = {sizeof(idt) - 1, //apunta al ultimo byte direccionable dentro de la idt
                             (uint32_t)&idt}; //direccion base de la idt
...
/* 
  en el offset tenemos que pasarle la direccion de la rutina de atencion que se ejecutara
  al recibir la interrupcion . _isr##numero sera la rutina correspondiente a la interrupcion con el numero ##numero

  No podemos definir la IDT de forma estatica, porque al momento de compilar no sabemos en que direccion
  quedara exactamente la _isr */
...
/*
    con el segsel y el offset me voy a armar una direccion logica, la cual sera la direccion de la rutina
    de atencion de la interrupcion. El kernel es el que siempre atendera las interrupciones asique el segmento
    sabemos que siempre sera el segmento de codigo de nivel 0, entonces el selector sera GDT_CODE_0_SEL
    
    Por otro lado, el dpl de cada uno de estos idt_entry dependera del nivel de privilegio del que puede disparar
    esa interrupcion, por ejemplo para disparar una syscall, eso lo hace el usuario, desde un programa por ejemplo,
    entonces ese caso deberia ser nivel 3.
     Pero por ejemplo una tecla del teclado, queremos que solo se pueda hacer con maximo nivel de privilegio, no
    quisieramos que un programa en nivel de usuario pueda emitir una interrupcion haciendose pasar por el teclado,
    entonces en ese caso quisieramos dpl 0.
*/
...
// como no podemos tener la idt preparada de antemano, tenemos que iniciarlizarla nosotros a mano : 
void idt_init() {
  // Las 20 primeras excepciones son del procesador, reservadas: 
  IDT_ENTRY0(0);
  IDT_ENTRY0(1);
  IDT_ENTRY0(2);
  IDT_ENTRY0(3);
  ...
  IDT_ENTRY0(32);
  IDT_ENTRY0(33);
  // a las interrupciones de reloj y teclado las repameamos a las direcciones 0x20 (32d) y 0x21 (33d) respectivamente
